...
### 0. 共用的日期解析 `parseMD(str)`

`wdFromDate()` 與 `resequenceDates()` 都要把 MM/DD 字串轉成 `Date`，抽成一支共用函式。

解析要**寬鬆吃進舊格式、嚴格擋掉會被誤讀的格式**：`10-21`、`10.21`、`10月21日`、`10/21(三)` 這些線上可能存在的舊寫法都要能正確解析，但 `2026-10-21` 不行——它會從中間匹配成「26 月 10 日」，`new Date(2026,25,10)` 滾成 2028-02-10 而**不是 Invalid Date**，`isNaN` 攔不住。若第 1 天是這種格式，按一次 ↑↓ 就會把 15 天全部改寫到二月並寫回 Firebase，全程無提示。
...
`wdFromDate()` 解析失敗回傳 `-1`（不是 `0`），渲染端用 `WD[wd]||''` 讓它留白——回 `0` 會讓無效日期顯示成看似正常的「一」。
