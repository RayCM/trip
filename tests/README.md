...
| `test-dayform.js` | 16 | `openDayForm()` / `saveDayForm()`：日期欄位唯讀的四種情境、驗證失敗時不寫回也不關視窗、存檔以 merge 保留未列在表單中的欄位 |
| `test-legacy.js` | 7 | 線上舊資料相容性：`t()` 讀得懂 `{zh,en,ja}` 物件、會被誤讀的日期格式必須被擋下 |
| `test-render.js` | 18 | 用 DOM stub 實際執行整段主 script 與 `renderAll()`，檢查三個分頁的渲染輸出、雨天備案與參考資料的空值守衛 |
| `test-sheet-import.js` | 63 | CSV 解析、列轉換、選用欄語意、差異計算、套用差異、差異視窗渲染 |
| `test-sheet-e2e.js` | 11 | 用真實試算表 fixture 跑完整條匯入流程，含冪等性與選用欄缺席 |
...
- **`tests/fixtures/sheet-sample.csv` 是 Google 試算表的副本。** 試算表若增減欄位或改欄位名稱，`test-sheet-import.js` 會紅——那是正確的警示，代表匯入功能的欄位對應要跟著更新，不是測試壞了。其中「雨天備案」與「參考資料」是選用欄（`SHEET_COLS` 第三個元素為 `true`），從試算表移除它們不會中止匯入，也不會清空 app 上已有的資料；必要欄缺席則仍會中止。更新 fixture：
